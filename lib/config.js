...
module.exports = config => {
  return {
    ...
    output: [
      ...
      config.targets.includes(TARGET.CJS) && {
        banner: config.banner,
        compact: true,
        name: config.name,
        file: `${config.outDir}/${config.fileName}.cjs.js`,
        format: 'cjs',
        exports: 'named',
    ...
    plugins: [
      ...
      config.parser === PARSER.BUBLE && buble(config.parserOptions),
      config.parser === PARSER.BABEL && babel(config.parserOptions || {
